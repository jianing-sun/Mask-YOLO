...
class RiceConfig(Config):
    ...
    NUM_CLASSES = 1 + 1  # Background + rice
...
class RiceDataset(utils.Dataset):
    ...
    def load_rice(self, dataset_dir, subset):
...
def train(model):
    ...
    dataset_train.load_rice(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = RiceDataset()
    dataset_val.load_rice(args.dataset, "val")
    dataset_val.prepare()
...
if __name__ == '__main__':
    ...
    dataset = RiceDataset()
    dataset.load_rice(RICE_DIR, "train")
